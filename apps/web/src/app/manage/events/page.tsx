...
export default function EventsPage() {
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--background)', display: 'flex', flexDirection: 'column' }}>
      <PageHeader
        title="Мероприятия"
        subtitle={`${events.length} событий${upcomingCount > 0 ? ` · ${upcomingCount} предстоящих` : ''}`}
        action={{ label: 'Добавить', icon: 'add', onClick: () => { setForm(BLANK); setShowForm(true) } }}
      />
      ...
      <Sheet open={showForm} onClose={() => setShowForm(false)} title="Новое мероприятие">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {Object.entries(TYPES).map(([k, [l, icon, c]]) => (
                <button key={k} onClick={() => setForm((p: any) => ({ ...p, type: k }))} style={{ flex: 1, padding: '12px', borderRadius: 12, border: `1px solid ${form.type === k ? c : 'rgba(255,255,255,0.1)'}`, background: form.type === k ? `${c}22` : 'rgba(255,255,255,0.04)', color: form.type === k ? c : 'var(--on-surface-variant)', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {Object.entries(PAY_TYPES).map(([k, v]) => (
                <button key={k} onClick={() => setForm((p: any) => ({ ...p, paymentType: k }))} style={{ flex: 1, padding: '10px 4px', borderRadius: 10, border: `1px solid ${form.paymentType === k ? '#8B5CF6' : 'rgba(255,255,255,0.1)'}`, background: form.paymentType === k ? 'rgba(139,92,246,0.15)' : 'rgba(255,255,255,0.04)', color: form.paymentType === k ? '#a78bfa' : 'var(--on-surface-variant)', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
                  {v}
                </button>
              ))}
            </div>
          ...
          <button onClick={() => create.mutate(form)} disabled={create.isPending} style={{ width: '100%', padding: '14px 0', borderRadius: 14, border: 'none', background: 'linear-gradient(135deg, #8B5CF6, #4cd7f6)', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', marginTop: 4 }}>
          ...
          return (
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
                {[['Время', `${selected.startTime}${selected.endTime ? ` — ${selected.endTime}` : ''}`], ['Тип оплаты', PAY_TYPES[selected.paymentType] ?? selected.paymentType], ['Сумма', selected.fixedAmount ? `${parseFloat(selected.fixedAmount).toLocaleString('ru')} ₽` : '—'], ['Комментарий', selected.comment || '—']].map(([k, v]) => v && v !== '—' ? (
              ...
              <button onClick={() => del.mutate(selected.id)} style={{ width: '100%', padding: '13px 0', borderRadius: 14, border: '1px solid rgba(244,63,94,0.3)', background: 'rgba(244,63,94,0.08)', color: '#F87171', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
