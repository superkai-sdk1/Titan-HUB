...
export default function DiscountsPage() {
  ...
  function openEdit(d: Discount) {
    setSelected(d)
    setForm({
      name: d.name, type: d.type, value: String(d.value), isActive: d.isActive, isAuto: d.isAuto,
      minQuantity: d.minQuantity != null ? String(d.minQuantity) : '',
      clientId: d.clientId ?? '', clientSearch: '', clientNickname: '',
    })
    setClientResults([])
    setShowCreate(true)
  }
  ...
  function selectClient(c: ClientResult) {
    setForm(f => ({ ...f, clientId: c.id, clientNickname: c.nickname, clientSearch: '' }))
    setClientResults([])
  }
          ...
          <div>
            ...
            {form.clientId && form.clientNickname ? (
              ...
              <div style={{ position: 'relative' }}>
                <input
                  style={INP}
                  placeholder="Поиск клиента по никнейму..."
                  value={form.clientSearch}
                  onChange={e => { setForm(f => ({ ...f, clientSearch: e.target.value })); searchClients(e.target.value) }}
                />
