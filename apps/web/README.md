...
│   │   ├── pos/                # Касса (список чеков, сплит: список + карточка)
│   │   │   └── [checkId]/      # Карточка открытого чека
│   │   ├── events/             # События (мероприятия)
│   │   ├── dashboard/          # Аналитика (обзор/финансы/расходы/игры/бар/игроки/персонал)
│   │   ├── ai/                 # TITAN AI (чат-ассистент)
...
│   │   ├── PullToRefreshContainer.tsx  # Локальный PTR для /pos и /dashboard
...
├── public/
│   ├── sw.js                   # Service Worker (CACHE_VERSION, стратегии кэша, Web Push)
...
На мобильных устройствах навигация выводится через `BottomNav.tsx` (4 пункта + центральная кнопка TITAN AI — FAB на `/dashboard`). На десктопе/планшете (≥1024px) — через `Sidebar.tsx` (раскладывается до 260px или сжимается до 72px, состояние сохраняется в `localStorage`).
...
### `/pos` — Касса
Список открытых чеков с pull-to-refresh (`PullToRefreshContainer`). При выборе чека разворачивается `CheckDetailView` (сплит на десктопе, полный экран на мобильном). Добавление позиций, тарифов, применение скидок/бонусов/сертификатов, оплата наличными/картой/переводом/депозитом/долгом. Поддерживает аренду зон с почасовой тарификацией. SSE-поток событий чека через `CheckChat.tsx` (`lib/sse.ts`). Открытие/закрытие смены — через `ShiftModals.tsx`.

### `/events` — События
Мероприятия с типами вечеров и почасовой оплатой (`event_hourly_rates`). Создание/редактирование/закрытие события.

### `/dashboard` — Аналитика
Вкладки: обзор, финансы, расходы, игры, бар, игроки, персонал. Диапазоны: 7 дней, 30 дней, текущий месяц, произвольный период. Расходы вынесены в отдельную вкладку `ExpensesTab`. Анимированные счётчики (`useCountUp`). Pull-to-refresh — собственный `PullToRefreshContainer` (глобальный PTR на `/dashboard` отключён).

### `/ai` — TITAN AI
Чат-интерфейс (`TitanAiChat.tsx`). Запросы на бэкенд (`POST /ai/chat`). Ассистент работает через провайдер Polza, модель `anthropic/claude-sonnet-4.6`, text-to-SQL по схеме БД (только `SELECT`). Иконка `titan_ai` — кастомная SVG в `Icon.tsx`. На экране `/ai` глобальный PTR отключён (есть внутренний скролл сообщений + прикреплённый снизу композер).
...
**Service Worker** (`public/sw.js`, текущая версия `CACHE_VERSION = 'v211'`):
...
Отключён на: `/login`, `/tablet*`, `/pos*`, `/dashboard*`, `/ai*` (у них собственные PTR или внутренний скролл).
...
**React Query** (`@tanstack/react-query`) используется для всех серверных запросов. `QueryClientProvider` монтируется в `app/providers.tsx`. Ключи запросов — вложенные массивы: `['shifts', 'current']`, `['auth', 'me']` и т.д.
